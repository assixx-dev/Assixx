...
| Halls (1:N from Area)          | `backend/src/nest/halls/` · `frontend/src/routes/(app)/(admin)/manage-halls/`                                               | DB: `halls.area_id` (a Hall belongs to one Area)                                                                                                                                           |
| Department → Hall (1:1)        | `backend/src/nest/departments/departments.service.ts` (`setDepartmentHall`) · DB trigger `trg_enforce_dept_hall_area_match` | A Department points to ≤ 1 Hall via `departments.hall_id`; trigger enforces `hall.area_id = dept.area_id` · [ADR-057](./infrastructure/adr/ADR-057-department-hall-1to1-hierarchy.md)      |
| Team → Hall (transitive)       | `backend/src/nest/teams/teams.service.ts` (`getTeamHallId`)                                                                 | Team inherits hall via `team.department.hall_id`; no own `team_halls` (dropped in migration 20260505221345432) · [ADR-057](./infrastructure/adr/ADR-057-department-hall-1to1-hierarchy.md) |
